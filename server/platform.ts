...
export const platform = {
  ...
  runAgent(agentId: string, userId: string, input: unknown) {
    return request<{
      ...
      body: JSON.stringify({
        user_id: userId,
        objective: "请直接输出一段适合显示在周报详情卡片中的中文自然语言文本，不要输出 JSON、表格或拆成多个独立结果区块。周报正文来自附件提取文本：先从最近一份历史周报附件中识别当时提出的后续计划，再与本次周报附件中的完成情况逐项对照，说明已完成、部分完成、未体现和计划外完成的内容及判断依据；如果没有历史附件或无法识别计划则明确说明。然后结合历史周报附件和已有评论评价本次内容，指出不足并给出具体、可执行的改进建议。",
        input,
        mode: "task",
